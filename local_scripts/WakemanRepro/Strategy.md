...
| 1 | raw vs. MNE maxwell_filter vs. Elekta MaxFilter difference | `03-maxwell_filtering.py` | `maxwell-filter` | ✅ **done for S10 run02** — `figures/fig1_maxfilter_comparison.py`, butterfly plot matching the paper's actual A/B/C structure and caption (1–40 Hz, -200 to 800 ms). Uses a local SSS run (calibration/cross_talk only, no app) since the brainlife.io app has no wiring for the shared calibration input yet; panel C (MNE − Elekta) isn't as flat as the paper's — missing tSSS/head_pos/MaxFilter-log bad channels (open question below) |
...
4. Panel C (MNE − Elekta difference) in our Fig 1 isn't as flat as the paper's — closing that gap needs tSSS (`st_duration`), movement compensation (`head_pos`, computed from cHPI), and Elekta's own MaxFilter-log bad-channel list, none of which we have yet. Worth pursuing for fidelity, or acceptable as-is for now?
5. Should `maxwell-filter` (the app) be hardened with the full documented recipe (`calibration`/`cross_talk`/`st_duration`/`origin`/`destination`/`head_pos`, all already declared in its config schema) rather than running with defaults? Its config already supports all of these — none are wired up by us yet.
