...
| 1 | raw vs. MNE maxwell_filter vs. Elekta MaxFilter difference | `03-maxwell_filtering.py` | `maxwell-filter` | ✅ **done for S10 run02, close match** — `figures/fig1_maxfilter_comparison.py`, butterfly plot matching the paper's actual A/B/C structure and caption (1–40 Hz, -200 to 800 ms). Uses a local SSS run (calibration/cross_talk + Elekta's exact origin, no app) since the brainlife.io app has no wiring for the shared calibration input yet. Panel C (MNE − Elekta) is now close to flat (evoked diff std 13.2 fT vs. Elekta's own 56.3 fT) — remaining gap traced to origin mismatch (fixed: MNE's `origin="auto"` fit converged ~30mm off from Elekta's documented origin) rather than tSSS/head_pos (tested, no help) or bad channels (MNE's auto-detection finds none). `destination="run4"` cross-run alignment not yet tried |
...
4. Panel C (MNE − Elekta difference) in our Fig 1 is now close to flat but not perfectly so — the remaining gap is most likely `destination="run4"` (Elekta's SSS output for run 2 is aligned to run 4's head position, ours isn't; ds000117's derivatives sidecar JSON documents this explicitly). Closing it needs run 4's data too. Worth pursuing, or good enough as-is?
5. Should `maxwell-filter` (the app) be hardened with the fuller documented recipe (`calibration`/`cross_talk`/explicit `origin`, all already declared in its config schema, confirmed to matter) rather than running with defaults (`origin="auto"` in particular converged ~30mm off from Elekta's own fit for S10 run02 — worth checking whether that's a one-off or systematic)?
