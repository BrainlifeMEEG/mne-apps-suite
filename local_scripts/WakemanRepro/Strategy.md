...
- **Phase 1 (fif2mne) and Phase 2 (maxwell-filter, Fig 1)**: done for S10 run02, see `run_pipeline.py` and `figures/fig1_maxfilter_comparison.py`. Along the way: fixed a real `maxwell-filter` bug (`head_pos` fed the wrong file type) and a real `bl-app-run-instanced.js` crash (in `BrainlifePipelineCLI`). Fig 1 is a butterfly plot (per-sensor traces, colored by sensor position), 1–40 Hz filtered, -200 to 800 ms, matching the paper's actual caption — confirmed directly against the published `03-maxwell_filtering.py` source that there's no `mne.chpi.filter_chpi()` call in the real pipeline (an earlier draft used it and got a superficially clean result for the wrong reason); the real recipe is `calibration`/`cross_talk`/`st_duration` (tSSS)/`origin`/`destination`/`head_pos` in `maxwell_filter()`, bad channels read from a MaxFilter log file, then a plain 1–40 Hz post-filter. We currently only supply calibration/cross_talk — see open questions.
...
| 1 | raw vs. MNE maxwell_filter vs. Elekta MaxFilter difference | `03-maxwell_filtering.py` | `maxwell-filter` | ✅ **done for S10 run02, close match** — `figures/fig1_maxfilter_comparison.py`, butterfly plot matching the paper's actual A/B/C structure and caption (1–40 Hz, -200 to 800 ms). Uses a local SSS run (calibration/cross_talk + Elekta's exact origin + destination aligned to run 4, no app) since the brainlife.io app has no wiring for these shared/cross-run inputs yet. Panel C (MNE − Elekta) is now essentially flat (evoked diff std 10.0 fT vs. Elekta's own 56.3 fT), matching the paper closely. Two fixes got there: explicit `origin=(0,8,37)mm` (MNE's `origin="auto"` fit had converged ~30mm off) and `destination=<run4's raw>` (cross-run alignment, matching Elekta's `trans=run4`). Movement compensation and MNE's auto-bad-channel detection were tested and don't help for this subject/run |
