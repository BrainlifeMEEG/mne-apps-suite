...
**Follow-up, same day: checked whether a different T1 source would close the gap -- it wouldn't.**
User's visual review of slices=[39,87,99,150] found the match still imperfect and asked what other
T1 images exist for this subject, correctly suspecting the background volume itself might be part
of the problem. Investigated properly: this dataset ships exactly one raw T1w acquisition per
subject (`sub-NN_ses-mri_acq-mprage_T1w.nii.gz`) -- present only as an unfetched git-annex
placeholder until this point, `git annex get` fixed that. Compared it against the
FreeSurfer-derivative `T1.mgz` already in use (from `ds000117/derivatives/freesurfer`, NOT
generated by this project): their affines and world-space volume centers are identical, confirming
the derivative genuinely is a conform of this same raw scan, not a different/misaligned image. Ran
a fresh `mri_convert --conform` on the raw file directly for a second check: same affine as the
derivative (spatial correspondence confirmed independently) but different voxel intensities (~41%
identical, mean abs diff ~4.8/255) -- the shipped derivative went through recon-all's fuller
intensity-normalization pipeline, a bare conform didn't; a contrast difference, not a positional
one. Conclusion: the T1.mgz in use is spatially correct for this subject, so no alternative local
T1 source was going to improve slice correspondence. The residual mismatch most likely reflects the
original 2018 paper having used a different (older) FreeSurfer version's own conform convention,
which this project has no way to reproduce exactly, not a wrong-image bug here.

**Decision: reverted to the user's originally-specified slices=[40, 100, 140, 180]**, rather than
keep iterating against a ceiling that direct investigation showed isn't fixable by trying harder
with the tools available. The FLASH-on-T1.mgz hybrid (surfaces + background choice) is unchanged --
only the panel indices reverted.
